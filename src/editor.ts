...
export class EditorState {
  ...
  select(key: number, layer = this.layer) {
    this.key = integer(key, 65, msg('field.physical'));
    this.layer = integer(layer, 2, msg('field.layer'));
  }
  applyDefinitions(edits: { index: number; definition: KeyDefinition }[]) {
    assert(this.profile, msg('error.editorProfile'));
    assert(Array.isArray(edits) && edits.length > 0 && edits.length <= 198, msg('error.editCount'));
    const next = this.profile.clone(),
      seen = new Set();
    for (const { index, definition } of edits) {
      integer(index, 197, msg('field.editableKey'));
    ...
    Profile.fromReports(next.reports);
    this.profile = next;
  }
  resetKey() {
  ...
  color(value: string, all = false) {
    ...
    for (let key = 0; key < 66; key++) if (all || key === this.key) this.profile.lights.set(color, key * 3);
  }
}
